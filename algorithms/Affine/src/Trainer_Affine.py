...
from algorithms.Affine.src.dataloaders import dataloader_factory
from algorithms.Affine.src.models import model_factory
...
class Trainer_Affine:
    def __init__(self, args, device, exp_idx):
        ...
        self.geo_classifier = Multilabel_Classifier(self.args.feature_dim, 10).to(self.device)
    ...
    def init_training(self):
        ...
        optimizer_params = list(self.model.parameters()) + list(self.classifier.parameters()) + list(self.geo_classifier.parameters())
    ...
    def train(self):
        ...
        for iteration in range(self.args.iterations):
            if (iteration % len(self.train_iter_loader)) == 0:
                self.train_iter_loader = iter(self.train_loader)
            samples, labels, geo_labels = self.train_iter_loader.next()
            samples, labels, geo_labels = samples.to(self.device), labels.to(self.device), geo_labels.to(self.device)
            features = self.model(samples)
            predicted_classes = self.classifier(features)
            predicted_geo_classes = self.geo_classifier(features)
            classification_loss = self.criterion(predicted_classes, labels)
            classification_geo_loss = self.multilabel_criterion(predicted_geo_classes, geo_labels)
            total_loss = classification_loss + 0.1 * classification_geo_loss
